...
function Get-InstalledEntry {
  ...
  foreach ($root in $roots) {
    $entry = Get-ItemProperty -Path $root -ErrorAction SilentlyContinue |
      Where-Object { $_.DisplayName -eq "NebulaSeek" } |
...
function Activate-App {
  ...
  if (-not $shell.AppActivate($Process.Id)) {
    throw "could not activate NebulaSeek"
  }
...
try {
  ...
  if (-not $installedExecutable) {
    throw "installed NebulaSeek executable was not found"
  }

  Assert-Pe -Path $installedExecutable -AllowedMachines @(0x8664)
  $appProcess = Start-Process -FilePath $installedExecutable -PassThru
  $expectedTitle = "NebulaSeek v$ExpectedVersion"
  $deadline = [DateTime]::UtcNow.AddSeconds(120)
  do {
    $appProcess.Refresh()
    if ($appProcess.HasExited) {
      throw "NebulaSeek exited before its main window was ready"
    }
  ...
  if (-not $appProcess.WaitForExit(30000)) {
    throw "NebulaSeek did not exit after close confirmation"
  }
  if ($appProcess.ExitCode -ne 0) {
    throw "NebulaSeek exited with code $($appProcess.ExitCode)"
  }
  ...
  Write-Output "Windows x64 installation acceptance passed for NebulaSeek $ExpectedVersion."
} finally {
  ...
  if ($null -ne (Get-InstalledEntry)) {
    throw "NebulaSeek remained installed after acceptance cleanup"
  }
}
