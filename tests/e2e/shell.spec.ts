...
test("automatic Harness startup, language switching, and status views fully load", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: "内核已就绪" })).toBeVisible();
  ...
  await expect(page.getByRole("heading", { name: "Core ready" })).toBeVisible();
  ...
  await expect(page.getByRole("heading", { name: "Core diagnostics" })).toBeVisible();
  ...
  await expect(page.getByRole("heading", { name: "Independent Core update" })).toBeVisible();
  await expect(page.getByRole("textbox", { name: "Manifest URL" })).toHaveCount(0);
  const harnessRepository = page.getByRole("textbox", { name: "Core repository" });
  ...
  await expect(page.getByRole("combobox", { name: "Core channel" })).toHaveValue("stable");
  await expect(page.getByRole("button", { name: "Restore bundled Core" })).toBeVisible();
