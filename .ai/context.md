...
- 历史首发标签已于 2026-09-22 从 `v1.0.0`、`v1.0.1`、`v1.0.2` 重新编号为 `v0.0.0`、`v0.0.1`、`v0.0.2`；当前仅保留 `v0.0.2` 历史预发布 Release，安装包内容未重建。历史上最后完成完整验证的是 `v1.1.27`；其质量门禁、四平台原生构建、Windows x64 安装交互、汇总发布和本机 ARM64 安装证据仍保留在 [验证基线](memory/verification.md#当前发布验收)，但不代表当前源码。下一次新发行从 `v0.1.6.1` 四段体系开始。防复发规则见 [发布手册](skills/release-workflow.md#最短反馈路径)。
...
- 新发行体系使用四段公开版本：锁定 Harness `0.1.6` 对应 Desktop `0.1.6.<修订号>`，当前默认示例为 `0.1.6.1`；三个 `v0.0.x` 标签只作历史归档，更新器只接受四段格式。
