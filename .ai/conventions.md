...
- 公开版本固定为四段数字：前三段等于工具链 lock 的 Harness 三段基础版本，第四段是从 `1` 开始递增的 Desktop 修订号；当前默认版本为 `0.1.5.2`。
...
- 社区仓库的 `docs/releases/<版本>.md` 是其公开 Release 正文的逐字归档（`gh release edit --notes-file` 直接使用），因此社区不会在其中加任何来源标记，合并时不会产生冲突。它们描述的是社区版发行事实，必须在合并后移除或替换为专版正文；专版 `docs/releases/` 只归档本仓库实际发布过的版本。社区的归档索引条目同样要换成专版事实。见社区 `c6693e3` 的书面规则。
- `.ai/` 与 CHANGELOG 的冲突一律保留专版侧，不采纳社区的验收证据与撤下清单；社区代码与脚本修复正常采纳。
- 合并带入发布关键路径（打包脚本、工作流、安装验收脚本）的改动时，按改动范围补跑本机验证，不因“只是同步上游”跳过。
