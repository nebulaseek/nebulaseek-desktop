...
暂时只忽略版本预发布标识中的 `alpha`、`beta`（含 `alpha.2`、`beta2` 等编号形式，大小写不敏感），`rc` 及其他类型仍可作为候选。仅更新候选受此过滤：当前已安装或安装包内已审计的内核不会因此被替换或降级。仓库模式在浅克隆后读取 `bin.dsh` 对应 CLI 的实际版本；命中这两种类型会在安装依赖和构建前跳过，并显示“已忽略”提示。
